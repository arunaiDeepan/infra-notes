...
>
> - **B Snowball Edge** is for moving data *to* AWS (migration not allowed here) and is temporary.
> - **C DataSync** transfers data between on-prem and cloud; it doesn't run compute locally.
> - **D Storage Gateway** gives on-prem access to *cloud* storage; it doesn't run EC2/DB locally.
...
> [!success]- Answer: B
> The live DB is local, but **RDS backups go to the Region**. If backups must also stay local, run the DB yourself on EC2 and use **EBS local snapshots**. (RDS *is* supported on Outposts racks, so D is wrong.)
...
> Small footprint per store → **servers**, which use an **LNI** to sit on the store LAN. Racks (A) need data-center power/space. Local Zones/Wavelength aren't in *your* building.
...
> DX gives predictable bandwidth/jitter for control-plane + VPC traffic. Internet works but is less predictable. Outposts always needs *some* link (so C is wrong).
...
| "Same control plane on-prem" | Outposts (ECS/EKS), not *Anywhere* |
