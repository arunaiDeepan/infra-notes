...
*Protecting data and access at every step.*
...
- **AWS Identity and Access Management (IAM):**
  ...
  - **`ECS Task Role`:** This is **critical**. It's the role attached to the container running your actual job. It should follow the **least privilege principle**, granting only permissions needed for *that specific job* (e.g., `s3:GetObject` for input, `s3:PutObject` for output).
...
*Ensuring jobs complete even if there are failures.*
...
*Selecting the right resources for the right job.*
...
- **Lifecycle:** Set your compute environment's `minimum vCPUs` to **0**. This ensures that when the queue is empty, Batch terminates *all* EC2 instances, saving 100% of the idle cost.
