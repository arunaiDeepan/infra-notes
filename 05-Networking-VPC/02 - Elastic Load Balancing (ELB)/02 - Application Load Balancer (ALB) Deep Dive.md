...
```json
{
  ...
  "Actions": [
    { "Type": "forward", "TargetGroupArn": "arn:aws:...:targetgroup/tg-api/..." }
  ]
}
```
...
> **Exam Tip:** "Capture detailed records of every request to the load balancer" -> **enable ALB access logs to S3**. For real-time API call auditing of the *ELB API* itself, that's **CloudTrail**.
