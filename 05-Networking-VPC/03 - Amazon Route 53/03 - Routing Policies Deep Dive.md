...
**How it works:** Returns **up to 8 healthy records** chosen at random, each optionally tied to its own **health check**. Unhealthy records are excluded from responses. It is **client-side load balancing / availability**, *not* a substitute for an actual load balancer.
...
**Remember:** Only **Simple** lacks health-check support; **Geoproximity** needs **Traffic Flow**; **Failover** *requires* a health check on the primary.
