...
| **Request tag** | A tag being set by *the current request* (e.g. `RunInstances --tags`) | `aws:RequestTag/<key>` |
...
- A single role + policy now powers access for every user - their permissions follow their *attributes*, not their role membership.
...
5. **Tag Policies** govern *which tags are allowed*; **SCPs** govern *whether actions can run without them*; **Config rules** flag the strays.
