...
See also: [01 - IAM Intro bits & bytes](01%20-%20IAM%20Intro%20bits%20%26%20bytes.md) · [22 - Secrets Manager vs SSM Parameter Store](22%20-%20Secrets%20Manager%20vs%20SSM%20Parameter%20Store.md) · [05 - IAM Scenarios](05%20-%20IAM%20Scenarios.md)
...
- [11. AWS Certificate Manager (ACM)](#11-aws-certificate-manager-acm)
- [12. CloudHSM - When KMS Isn't Enough](#12-cloudhsm---when-kms-isnt-enough)
...
| **External key (XKS)** | Stored in *your* HSM outside AWS | $1/month + API | You | ✅ Yes |
...
The `Principal: root` here means "delegate to IAM" - IAM policies in this account can grant `kms:*`. **Do not** confuse this with the AWS root *user*.
...
**Grants** are an alternative to key policies for granting *temporary, programmatic* access. Common pattern: an AWS service (EBS, RDS) needs to use a CMK on your behalf during a lifecycle event.
...
Related but distinct from KMS - **ACM** issues and manages **TLS certificates** (for HTTPS).

| Feature | Detail |
| :--- | :--- |
| **Public certs** | Free, issued by Amazon Trust Services. Auto-renew. |
| **Private certs** | Use ACM Private CA (paid). For mTLS / internal services. |
| **Where deployable** | ALB, NLB (since 2018), CloudFront, API Gateway, App Runner, Elastic Beanstalk (via ALB). |
| **Per-region** | Cert lives in one Region. EXCEPTION: **CloudFront** requires the cert in **`us-east-1`**. |
| **Wildcards** | Supported (`*.example.com`). |
| **Auto-renewal** | If DNS validation is set up, ACM renews 60 days before expiry. Public certs only. |
| **Importing your own** | Allowed, but no auto-renewal - you remember. |

> EC2 instances can't directly use an ACM public cert - you'd attach the cert to a load balancer (ALB / NLB) in front of EC2. Private ACM certs *can* be installed on EC2.

[⬆ Back to top](#table-of-contents)

---

## 12. CloudHSM - When KMS Isn't Enough
...
10. **ACM certificates for CloudFront must be in `us-east-1`.** For ALB / API Gateway, in the same region as the resource.
11. **Public ACM certs auto-renew with DNS validation.** Imported certs do not.
12. **KMS or CloudHSM?** Default KMS. CloudHSM only if the question says **single-tenant** or **PKCS #11** or specific regulatory ask.
...
- **ACM** issues TLS certs. CloudFront certs live in `us-east-1`; all others are regional.
