...
An **AZ** is one or more *discrete data centers* with redundant power, networking, and connectivity, located within a Region.
...
While Regions and AZs host your *infrastructure*, **Edge Locations** serve your *users*.
...
| **Regional** (per-Region resource) | VPC, EC2, EBS, RDS, DynamoDB, SQS, SNS, S3 *(bucket name is global, data is regional)*, Lambda, ECS, EKS, KMS keys, Secrets Manager, ACM (per region, except the CloudFront special case) |
