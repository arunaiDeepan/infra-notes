...
> How each AWS service actually behaves when it runs on an Outpost: EC2, EBS (and local snapshots), S3 on Outposts (access points + DataSync), RDS, ElastiCache/EMR, ECS, and EKS local-vs-extended clusters. The exam tests the _differences_ from the Region version, not the basics of each service.
...
- Automated **backups and snapshots are stored in the parent Region's S3** (so RDS backups _do_ traverse the service link — note this for data-residency questions about the _database engine vs. backups_).
...
> **Subtlety:** RDS on Outposts keeps the _live database_ local, but its _backups_ go to the Region. If a question demands that **even backups** stay on-prem, RDS on Outposts may not satisfy it — consider self-managed DB on EC2 + EBS local snapshots.
...
> **Exam trap:** "Run containers on-prem with the same control plane as AWS" → **ECS/EKS on Outposts**, _not_ ECS Anywhere/EKS Anywhere (those are for **non-Outposts** customer hardware).
