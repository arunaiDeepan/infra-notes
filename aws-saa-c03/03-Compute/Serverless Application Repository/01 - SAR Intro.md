...
> The **Serverless Application Repository (SAR)** is a **managed catalog of reusable serverless applications**. Think of it as an _"app store for serverless"_: developers **publish** ready-made apps (built on AWS SAM), and anyone with permission can **discover and deploy** them in a few clicks. Deployment is just a **CloudFormation stack** under the hood, and **SAR itself is free** — you only pay for the resources the app creates (Lambda, API Gateway, DynamoDB, etc.).
...
| **Application policy** | A **resource-based policy** controlling _who_ (which accounts / org / everyone) can find and deploy the app.     |
| **Nested application** | A SAR app referenced _inside another_ SAM template via `AWS::Serverless::Application` — composition of apps.     |
...
| Deploying                     | Free (the _deployed resources_ incur normal AWS charges) |
...
These "repository/catalog" services are a classic source of distractor answers. Memorize what each one _stores_.
