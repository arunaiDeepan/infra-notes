...
**The single most important framing for the exam:** every container question is really asking _"which orchestrator, and where does the compute live?"_
...
Both orchestrators decouple the _control plane_ from the _data plane_ (where containers actually run):
...
- **ECS Anywhere** = AWS still orchestrates from the cloud; only your _servers_ are on-prem.
- **EKS Anywhere** = an installable product to run a _complete_ Kubernetes cluster (control plane included) on-prem, optionally viewable in AWS via the EKS Connector.
- **EKS Distro** = the raw open-source Kubernetes distribution (same versions/patches as EKS) that you assemble and operate entirely yourself. EKS Anywhere is _built on top of_ it.
