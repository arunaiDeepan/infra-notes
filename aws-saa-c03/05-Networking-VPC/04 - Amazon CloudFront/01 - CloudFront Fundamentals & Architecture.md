...
CloudFront is one of the most frequently tested services in the SAA-C03 networking and performance domains. Understanding _how a request flows through the network_ is the key to answering latency and caching questions correctly.
...
- Choosing a cheaper class means viewers in **excluded regions** are routed to the **nearest _included_ edge location** — they still get content, just with higher latency.
- DNS routing still works globally; only the _edge serving location_ is constrained.
