...
- **VPC A** is _associated_ with `RT-Prod` → when traffic from VPC A hits the TGW, the TGW looks up `RT-Prod` to decide the next hop.
- **VPC Shared** _propagates_ its CIDR into `RT-Prod` → so `RT-Prod` knows how to reach the Shared VPC.
