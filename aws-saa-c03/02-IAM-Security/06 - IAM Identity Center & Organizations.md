...
- **Management account** (formerly "master account") - the one that _creates_ the organization. Holds billing, applies policies, can never be removed without dissolving the org.
...
Policies (SCPs, tag policies, etc.) attach to **the root**, **an OU**, or **an individual account**, and **flow downward** - every account inherits the union of all policies above it. (Inheritance is _intersection_ for SCPs - see below.)
...
If any SCP in the chain _denies_ an action, the action is denied for everyone in the account - including the **root user** of that account. The only principal SCPs don't apply to is the **management account itself**, which is why you never run workloads there.
...
| **Allow list**              | Detach `FullAWSAccess`, then _grant_ specific services via Allow SCPs at each level                        | Strict regulated environments - every service must be explicitly enabled          |
...
A **permission set** is a template that defines _what_ a user can do once they assume a role in a target account.
...
The source of truth for _who exists_. Pick one (and only one per Identity Center instance):
