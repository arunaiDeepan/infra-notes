...
| Forgetting the trailing `/*` in object ARN                      | Without `/*` you only target the _folder marker_ (which doesn't exist), not files inside | Use `${aws:username}/*`                               |
...
```json
{
  ...
  "Statement": [
    ...
    },
    {
      ...
      "Effect": "Deny",
      "Action": ["s3:PutObject", "ec2:StartInstances", "iam:CreateAccessKey"],
      "Resource": "*",
      "Condition": {
        "BoolIfExists": {
