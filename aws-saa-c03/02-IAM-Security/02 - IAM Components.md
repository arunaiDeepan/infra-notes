...
An **IAM User** is a _permanent identity_ representing a **person** or **application** that needs long-term access to AWS.
...
A **Policy** is the document that _defines permissions_. It answers: _"Who can do What on Which resource?"_

- **Types:**
  - **AWS Managed:** Created by AWS (e.g., `AdministratorAccess`, `ReadOnlyAccess`).
  - **Customer Managed:** You create and manage them.
  - **Inline:** A policy embedded directly _inside_ a single User or Group (not recommended-hard to audit).

### 🎭 IAM Role

A **Role** is an identity _intended to be assumed_ temporarily. It has no long-term credentials (no password, no access key).
...
```json
{
  ...
  "Statement": [
    // Array of individual permissions (can have multiple)
    {
...
> **If there's an _Explicit_ `Deny` policy = `Deny` (wins over everything).**
...
This is the most misunderstood part. **Groups do NOT nest**. Instead, permissions are _aggregated_.
...
You will often need an IAM User who can do everything _except_ the Root-only tasks. This is an **Admin IAM User**.
...
_A:_ **Deny** wins. Explicit Deny overrides any Allow.

**Q2:** You need to give 20 developers permission to launch EC2 instances, but only from the office IP. What's the right way?  
_A:_ Create one Customer Managed Policy with `ec2:RunInstances` and the `Condition` on `aws:SourceIp`. Attach that policy to a `Developers` Group. Add all 20 users to the Group.

**Q3:** Can Root User be restricted by an IAM policy?  
_A:_ **No.** Root bypasses all IAM policies. That's why it's so dangerous.

**Q4:** What happens if a policy is attached to a user AND to a group the user belongs to?  
_A:_ They combine (union) of all permissions. Deny still overrides.
