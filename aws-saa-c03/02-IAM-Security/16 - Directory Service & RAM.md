...
The alternative - copying or recreating in each account - wastes money and causes drift. RAM lets one account _own_ the resource while others _use_ it as if it were their own.
...
11. **RAM ≠ cross-account IAM roles.** RAM shares _resources_ (use them as-is); roles let _principals_ assume identity in another account.
